...
describe("[Challenge] The rewarder", function () {
    ...
    before(async function () {
        ...
        [deployer, alice, bob, charlie, david, player] =
        ...
        flashLoanPool = await FlashLoanerPoolFactory.deploy(
            liquidityToken.address
        );

        // Set initial token balance of the pool offering flash loans
        await liquidityToken.transfer(
            flashLoanPool.address,
            TOKENS_IN_LENDER_POOL
        );

        rewarderPool = await TheRewarderPoolFactory.deploy(
            liquidityToken.address
        );
        rewardToken = RewardTokenFactory.attach(
            await rewarderPool.rewardToken()
        );
    ...
    it("Execution", async function () {
        ...
        attacker = await attackContract.deploy(
            flashLoanPool.address,
            rewarderPool.address,
            liquidityToken.address,
            rewardToken.address
