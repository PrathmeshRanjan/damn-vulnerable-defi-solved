...
describe("[Challenge] Free Rider", function () {
    ...
    before(async function () {
        ...
        // Deploy WETH
        weth = await (
            await ethers.getContractFactory("WETH", deployer)
        ).deploy();
    ...
    it("Execution", async function () {
        ...
        const FreeRider = await ethers.getContractFactory("FreeRider", player);
    ...
    after(async function () {
        ...
        expect(await ethers.provider.getBalance(devsContract.address)).to.be.eq(
            0
        );
    });
});
